// subPackages/pages/transaction/cate-bill/index.ts
...
import { categoryBillList } from '../../../../api/category'
...
Page({
	...
	async handleTransactionList(type) {
		...
		let res: any = await categoryBillList(data)
	...
	onLoad({ date, yearMonthMoreActive, type, categoryId, bookId, userId, categoryName }) {
		...
		this.setData({
			title:yearMonthMoreActive=='1'?'分类支出':'分类收入',
