Component({
  ...
  methods: {
      ...
      display: function (year, month, date) {
          this.setData({
              ...
              month,
              date,
              title:`${year} / ${this.zero(month)}` ///+ '月'//year + '年' + 
          })
      ...
      createDays: function (year, month) {
        // console.log('接收的收支数据：', this.properties.list);
        let thisMonthDays = [];
        // 获取当前月份天数
        ...
        for (let i = 1; i <= days; i++) {
          ...
          // 匹配当前日期的收支数据（注意 day 可能是 "2" 或 "02"，需统一格式）
          const currentDay = this.zero(i); // 转为带零格式，如 2 → "02"
        //   console.log(currentDay,123)
          // 从 list 中找对应日期的收支
					// console.log(this.properties.list)
          const dayData = this.properties.list.find(item => {
            // 兼容 day 是 "2" 或 "02" 的情况
          ...
          if (dayData) {
            ...
            expense = dayData.expense || '0.00';
          }
  
          // 组装日期项（新增 income/expense 字段）
          thisMonthDays.push({
            checked: false,
            date: i, // 数字格式（如 2）
            ...
            balance: (Number(income) - Number(expense)).toFixed(2) // 可选：当日结余
          });
        }
  console.log(thisMonthDays)
        ...
        // console.log('带收支的日期数组：', thisMonthDays);
      },
      //获取当月空出的天数
